...
static const int sidepad                 = 180;  /* horizontal padding of bar */
...
static const char *fonts[]   = { 
      "JetBrainsMono Nerd Font:size=9:style=Bold", // Primary. For text
      "FontAwesome:size=9", // Fallback. For gylphs and icons
};
...
#if BAR_ALPHA_PATCH
static const unsigned int baralpha = 255; // transparency of the bar
static const unsigned int borderalpha = OPAQUE; // transparency of borders
...
static const char *const autostart[] = {
	"picom", NULL,
	"/home/username/.fehbg", "-c", NULL, // replace "username" with your username
	"slstatus", NULL,
	"dunst", NULL,
	"/home/username/scripts/mousepad", "-c", NULL, // replace "username" with your username 
	NULL /* terminate */
};
...
#endif // NAMETAG_PATCH
{
	[DEFAULT_TAGS]        = { "lorem", "ipsum", "dolor", "sit", "amet", "consectetur", "adipiscing", "elit", "Integer" },
...
static const char *xkb_layouts[]  = {
	"en", // how the tray looks in English
	"ar", // how the tray looks in Arabic
};
#endif // XKB_PATCH
...
static const char *mutecmd[] = { "sh", "-c",
    "pactl set-sink-mute 0 toggle && "
    "MUTED=$(pactl get-sink-mute 0 | grep -o 'yes\\|no') && "
    "if [ \"$MUTED\" = \"yes\" ]; then "
    "notify-send -i ~/scripts/pic/mute.png -r 1001 'Audio Muted' 'Sound is muted'; "
    "else "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i ~/scripts/pic/mute.png -r 1001 'Audio Unmuted' \"$VOL\"; "
    "fi", NULL };

static const char *volupcmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 +5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i ~/scripts/pic/volume.png -r 1002 'Volume Up' \"$VOL\"", NULL };

static const char *voldowncmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 -5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i ~/scripts/pic/volume.png -r 1002 'Volume Down' \"$VOL\"", NULL };

// Alternative with different icon styles:
/*
static const char *mutecmd[] = { "sh", "-c",
    "pactl set-sink-mute 0 toggle && "
    "MUTED=$(pactl get-sink-mute 0 | grep -o 'yes\\|no') && "
    "if [ \"$MUTED\" = \"yes\" ]; then "
    "notify-send -i audio-volume-muted -r 1001 '🚫🔊 Audio Muted' 'Sound is muted'; "
    "else "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i audio-volume-high -r 1001 '✅🔊 Audio Unmuted' \"Volume: $VOL\"; "
    "fi", NULL };

static const char *volupcmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 +5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i audio-volume-high -r 1002 '📢⬆️ Volume Up' \"Volume: $VOL\"", NULL };

static const char *voldowncmd[] = { "sh", "-c",
    "pactl set-sink-volume 0 -5% && "
    "VOL=$(pactl get-sink-volume 0 | grep -oP '\\d+%' | head -1) && "
    "notify-send -i audio-volume-low -r 1002 '📢⬇️ Volume Down' \"Volume: $VOL\"", NULL };
*/
...
static const Key keys[] = {
    ...
    { 0, XF86XK_MonBrightnessUp,   spawn, SHCMD("brightnessctl s 10%+") }, // brightness up 
    { 0, XF86XK_MonBrightnessDown, spawn, SHCMD("brightnessctl s 10%-") }, // brightness down
	{ 0,			        XK_Print,  spawn,	   {.v = prtscrcmd } }, // screenshot
	{ Mod4Mask, XK_space, spawn, SHCMD("setxkbmap us,iq -option grp:win_space_toggle") }, // to change between languages
	{ MODKEY|ShiftMask,		XK_n,      spawn,       {.v = (const char*[]){ "notes", NULL } } }, // for quick notes
	{ MODKEY,				XK_v,      spawn,       {.v = (const char*[]){ "cliphist", "sel", NULL } } }, // clipboard history
	{ MODKEY,				XK_c,      spawn,       {.v = (const char*[]){ "cliphist", "add", NULL } } }, // add to clipboard
	{ MODKEY|ControlMask,	XK_a,      spawn,       {.v = (const char*[]){ "audiooutputswitch", NULL } } }, // Output swticher
	{ MODKEY|ShiftMask,		XK_d,      spawn,       {.v = (const char*[]){ "killer", NULL } } }, // process killer
    { MODKEY|ShiftMask,		XK_w,      spawn,		{.v = (const char*[]){ "wallpaper", NULL } } }, // wallpaper changer
    { MODKEY|ShiftMask,		XK_v,      spawn,		{.v = (const char*[]){ "playvid", NULL } } }, // vid player
    { MODKEY|ShiftMask,		XK_b,      spawn,		{.v = (const char*[]){ "firefox", NULL } } }, // browser
};
